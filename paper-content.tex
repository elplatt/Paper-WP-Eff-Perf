...
some networks, they perform worst on self-organized
networks.
...
For larger projects, these calculations are impractical and we thus employed sampling to determine mean path length and mean
min-cut.
...
(results are similar with in-degree skewness).
...
The association of low-degree with both high performance and efficiency is compelling,
...
designed to satisfy several properties in real-world collaborations,
...
behavioral norms interact with network structure, which could be used to ensure that behavioral norms
and network structure are appropriately aligned.

\section{Conclusion}
\label{sec:conclusion}

In this paper, we have described the relationship between the structural properties of WikiProject
coeditor networks and their performance and efficiency.
...
While additional work is needed to determine causal relationships and the generalizability of our results,
...
real-world network. Our results suggest that the success of large-scale collaborations may be aided by greater decentralization,
